...
export default class SignInPage extends Component {
  ...
  _Login = async () => {
    ...
    fetch("http://hb.jgy-tec.com/token", {
      method: "POST",
      headers: {
      ...
      .then(responseJson => {
        ...
        if (responseJson.access_token) {
          ...
          if (Platform.OS == "ios") {
            AlertIOS.alert("提示", responseJson.error, [
  ...
  _storeData = async () => {
    try {
      await AsyncStorage.setItem(
        "UserInfo",
        JSON.stringify({ username: this.state.username, pwd: this.state.pwd })
      );
    } catch (error) {
      // Error saving data
    }
      ...
      >
        <View style={styles.editGroup}>
          ...
          <View style={styles.editView3}>
            <Switch
              onTintColor={this.state.remainUserInfo ? "#b3d4f5" : "#acacac"}
              tintColor="#acacac"
              thumbTintColor={this.state.remainUserInfo ? "#3281f1" : "#ebebeb"}
              value={this.state.remainUserInfo}
              onValueChange={value => {
                console.log(value);
                this.setState({
