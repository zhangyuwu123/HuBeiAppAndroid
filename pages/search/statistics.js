...
export default class Statistics extends Component {
    ...
    fetch(this.state.url, {
      ...
      .then(response => {
        if (response.status == 401) {
          ...
          this.addNavis(
            this.state.bridgeList[0].XZQHBM,
            this.state.bridgeList[0].XZQHMC
          );
          this.setState({
            bridgeList: this.state.bridgeList,
            navis: this.state.navis
          });
          this.setSummarisByArea(
            this.state.bridgeList[0].XZQHBM,
            this.state.bridgeList[0].XZQHMC
          );
        }
  ...
  setSummarisByArea(XZQHBM, XZQHMC) {
    ...
    let url =
      this.state.url == "dxgc" || this.state.url == "zxgc"
        ? this.state.url + "&xzqhbm=" + XZQHBM
        : this.state.url + "?xzqhbm=" + XZQHBM;
    console.log(url);
    fetch(url, {
      method: "POST",
      ...
      .then(response => {
        if (response.status == 401) {
          ...
          if (true) {
            ...
            this.setState({
              bridgeList: this.state.summaryData,
              navis: this.state.navis
            });
            this.setOptionByArea();
          }
    ...
    >
      <View style={styles.itemContainer}>
        <View style={styles.zt}>
          <Text style={styles.XZQHMC}>{item.XZQHMC}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.TCount}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.WWCCount}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.PFZTZ}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.WCZTZ}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.ZYTZ}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.WCZYTZ}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.DFTZ}</Text>
        </View>
        <View style={styles.zt}>
          <Text>{item.WCDFZC}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
  ...
  customNavi() {
    ...
    this.state.navis.forEach((item, index) => {
      cusNavi.push(
        <TouchableOpacity
          key={item.XZQHBM}
          onPress={() => this.getNextSummary(item)}
        >
          <View>
            <Text style={styles.cusNaviTxt}>
              {" "}
              {item.XZQHMC} {index + 1 < this.state.navis.length ? ">" : ""}
            </Text>
          </View>
        </TouchableOpacity>
      );
  ...
  render() {
    return (
      <ScrollView>
        <View style={styles.cusNavi}>{this.customNavi()}</View>
        <ScrollView
