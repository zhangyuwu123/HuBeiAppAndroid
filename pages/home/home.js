...
export default class App extends Component {
  ...
  onMySettings = () => {
    this.props.navigation.navigate('MySettingsManage', { userInfo: this.state.userInfo })
  }
  onBridgeDetail = (item) => {
  ...
  _onMarkerPressTemp = (item) => {
    ...
    let makerImg = 'http://bpic.588ku.com/element_origin_min_pic/18/06/10/217da78e9f80484b354afea9bb8a6538.jpg'
    ...
    return (
      <View style={StyleSheet.absoluteFill}>
        ...
        <View style={styles.makerContainer} ref="makerContainer" >
          <Image style={styles.makerImg} source={{ uri: this.state.makerImg }} />
          <View style={styles.detail}>
            <View>
              <Text style={styles.makerRouter}>{this.state.makerRouter}</Text>
              <Text style={styles.makerTitle}>{this.state.makerTitle}</Text>
              <Text style={styles.makerTime}>{this.formatDate(this.state.makerTime)}</Text>
            </View>
